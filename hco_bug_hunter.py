...
def open_youtube():
    """Open the HCO YouTube channel using Android Intent in Termux.
    Falls back to Python's webbrowser if Android's am command is unavailable.
    """
    try:
        # Termux/Android: ask Android to open the URL in the appropriate app.
        result = os.system(
            'am start -a android.intent.action.VIEW '
            '-d "https://youtube.com/@hackers_colony_termux?si=gJjJ3pKGkPpk1Qkw" '
            '> /dev/null 2>&1'
        )
        if result == 0:
            return True
    ...
    try:
        return bool(webbrowser.open(YOUTUBE))
    except Exception:
        return False
...
def lock_screen():
    ...
    for n in range(9,0,-1):
        print(f"\r{c(f'              {n}...',GREEN,True)}",end="",flush=True)
        time.sleep(1)
    print()
    ...
    if open_youtube():
        ...
        print(c("\n⚠ Could not open YouTube automatically.",YELLOW,True))
        print(c("Open the channel manually:",WHITE))
        print(YOUTUBE)
